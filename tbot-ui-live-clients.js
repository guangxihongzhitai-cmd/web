(function () {
  ...
  function renderClients() {
    ...
    state.clients.forEach(function (client) {
      ...
      var meta = document.createElement("span"); meta.textContent = [client.phone_display || client.phone, client.country].filter(Boolean).join(" 路 ");
  ...
  function renderDetail(data) {
    ...
    if (title) title.textContent = client ? ((client.name || client.phone_display || client.phone) + " 路 " + (client.phone_display || client.phone)) : "Select a client";
    ...
    messages.forEach(function (message) {
      ...
      var meta = document.createElement("div"); meta.className = "client-message-meta"; meta.textContent = [message.ts_display || message.ts || "", message.direction || "", message.sender_label || "", message.dispatch_id ? ("dispatch=" + message.dispatch_id) : "", message.message_id ? ("msg=" + message.message_id) : ""].filter(Boolean).join(" 路 ");
  ...
  function selectClient(phone) {
    state.selected = phone; renderClients(); setMessage("Loading conversation鈥?);
